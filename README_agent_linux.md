...
cp agent_linux.py /tmp/agent_linux.py

# Copy config template and create actual config
cp etc_overwatch_config.cfg.example /tmp/config.cfg
...
scp overwatch-runner root@target-server:/usr/local/bin/
...
```bash
./scripts/deploy_nexhub_agents.sh /tmp/servers.txt
